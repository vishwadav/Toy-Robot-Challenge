...
public class RobotSimulateService {
    ...
    public RobotModel placeRobot(String positionInput) {
        ...
        RobotModel robotModel = new RobotModel(Integer.parseInt(coordinatesList[0]),Integer.parseInt(coordinatesList[1]),coordinatesList[2]);
        return robotModel;
    }
    ...
    public boolean validateRobotCommand(String input) {
        ...
        /* Verify if command matches the PLACE command using regex pattern ^PLACE\s[0-4],[0-4],(NORTH|SOUTH|EAST|WEST)$
              OR
           Other commands i.e MOVE, LEFT, RIGHT, REPORT, STOP
         */

        if(input.matches(RobotCommonConstants.PLACE_COMMAND_REGEX) || Arrays.stream(commands).anyMatch(input::matches)) {
            return true;
        } else {
            return false;
        }
    ...
    public void report(RobotModel robotModel) {
        //REPORT will display the robot position and direction.
        //Null check for the robotModel verifies if a PLACE command executed initially
        if(robotModel != null) {
